...
class pSp(nn.Module):
    ...
    def set_encoder(self):
        if self.opts.encoder_type == 'GradualStyleEncoder':
            ...
            raise Exception(f"{self.opts.encoder_type} is not a valid encoder type")
        return encoder

    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            print(f'Loading e4e over the pSp framework from checkpoint: {self.opts.checkpoint_path}')
    ...
    def forward(self, x, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
        ...
        if return_latents:
            ...
            return images, result_latent  # <- Always return latent now for downstream usage
